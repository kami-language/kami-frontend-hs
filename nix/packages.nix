{
  ...
  ourHaskell = let
    # https://github.com/pwm/kami-frontend-hs#direct-hackagegithub-dependencies
    ...
    manual = hfinal: hprev: {
      ...
      in
        confPkg (hprev.callCabal2nix "kami-frontend-hs" cleanSource {});
    };
  in
  ...
  ghc = ourHaskell.ghc.withPackages (_:
    hlib.getHaskellBuildInputs (
      ...
      hlib.doBenchmark ourHaskell.kami-frontend-hs
    ));
...
in {
  ...
  bin = ourHaskell.kami-frontend-hs;

  shell = pkgs.buildEnv {
    name = "kami-frontend-hs-env";
